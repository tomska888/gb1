...
<template>
  <div>
        ...
        <div class="d-flex justify-content-between align-items-start">
          <div class="me-3 flex-grow-1">
            <div class="d-flex align-items-center gap-2">
              ...
              <h5
                class="mb-1"
                :class="{ 'text-decoration-line-through': goal.status === 'completed' }"
              >
                {{ goal.title }}
              </h5>
              ...
              <span
                v-for="t in splitTags(goal.tags)"
                :key="t"
          ...
          <div class="ms-2 d-flex flex-column align-items-end gap-2" style="min-width: 220px">
            <div class="d-flex align-items-center gap-2">
              ...
              <button v-else class="btn btn-sm btn-outline-secondary" @click="reopen(goal)">
                Reopen
              </button>
            </div>
    ...
    <div class="d-flex justify-content-between align-items-center mt-3">
      ...
      <div class="btn-group">
        <button class="btn btn-outline-secondary" :disabled="goalStore.page <= 1" @click="prevPage">
          Prev
        </button>
        <button
          class="btn btn-outline-secondary"
          :disabled="goalStore.page >= goalStore.totalPages"
          @click="nextPage"
        >
          Next
        </button>
...
function setStatus(s: StatusTab) {
  status.value = s
  goalStore.page = 1
  fetchList()
  loadCounters()
...
watch([debouncedSearch, categoryFilter, sortKey], () => {
  goalStore.page = 1
  fetchList()
  loadCounters()
})
...
function startEdit(g: Goal) {
  ...
  targetDate.value = g.targetDate ? g.targetDate.slice(0, 10) : null
...
async function onStatusChange(e: Event, g: Goal) {
  ...
  if (v === g.status) return
  try {
    await goalStore.updateGoal(g.id, { status: v })
...
async function markCompleted(g: Goal) {
  try {
    await goalStore.updateGoal(g.id, { status: 'completed' })
...
async function reopen(g: Goal) {
  try {
    await goalStore.updateGoal(g.id, { status: 'in_progress' })
