...
async function createUserAndToken(email = uniqEmail('user'), password = 'password123') {
...
async function shareGoal(ownerToken: string, goalId: number, email: string, permissions: 'view' | 'checkin' = 'checkin') {
  const res = await request(app)
...
describe('Collab Router – extra coverage (isolated cases)', () => {
  it('GET /goals/:goalId/shares returns list for owner, 403 for buddy', async () => {
    ...
    expect(ok.body[0]).toMatchObject({ buddy_id: expect.any(Number), email: buddy.email });
  ...
  it('DELETE /goals/:goalId/share/:buddyId revokes access and clears dependent data', async () => {
    ...
    expect(del.body).toMatchObject({ ok: true, clearedMessages: true, clearedBuddyCheckins: true });
  ...
  it('GET /owners aggregates owners for buddy and shows goal_count', async () => {
    ...
    const owners = await request(app).get('/api/collab/owners').set('Authorization', `Bearer ${buddy.token}`);
  ...
  it('GET /goals/shared supports ownerId, q/category filters, and every sort branch', async () => {
    ...
    for (const sort of ['created_asc','created_desc','target_asc','target_desc','title_asc','title_desc'] as const) {
