...
app.get("/", (req: Request, res: Response) => {
  res.status(200).json({
    message: `Welcome to the GoalBuddy API! Environment: ${process.env.NODE_ENV || 'production'}. ` +
...
if (process.env.NODE_ENV !== "test") {
  app.listen(port, host, () => {
    ...
    console.log(`Database: ${process.env.DATABASE_URL?.includes('localhost') ? 'Local PostgreSQL' : 'Neon Cloud'}`);
  });
}
